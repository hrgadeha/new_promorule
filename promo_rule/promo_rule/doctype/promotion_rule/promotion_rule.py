...
def apply_promo_rule(name,items,pos_profile):
	...
	for pro in promorules:
		p = frappe.get_doc("Promotion rule", pro)
		if p.special_promo:
			calculate_special_discount(doc,p)
	for x in doc['items']:
		for pro in promorules:    #check in all promo rules if its applicable to that item .
			...
			if p.to_date < now_datetime().date() or p.from_date > now_datetime().date() or not p.is_active:
				...
				continue
			# if p.special_promo:
			# 	calculate_special_discount(x,p)
...
def calculate_special_discount(doc,p):
	if p.base in brandlist:
		total_qty = brandlist.get(p.base, 0)
		total_amount =brandamout.get(p.base, 0)
		frappe.errprint("target item present")
		frappe.errprint(brandlist.get(p.base, 0))
		frappe.errprint(brandamout.get(p.base, 0))
		if total_amount > p.for_amount:
			frappe.errprint("discount to be applied ")
			if total_qty <= p.base_qty:
				frappe.errprint("discount to be applied is total amount minus for_amount")
				dis = total_amount - p.for_amount
				frappe.errprint(dis)
				min_amount_list[p.base] = dis
			if total_qty > p.base_qty:
				max_allowed_dis = 0
				counter = 1
				frappe.errprint("total qty greater")
				for x in doc['items']:    #count quantity of  all  of the  brands and item group in cart  
					# frappe.errprint(x['brand'] + "\n")
					if x['brand'] == p.base:
						# frappe.errprint(p.base_qty)
						# frappe.errprint(counter)
						if counter > p.base_qty:
							frappe.errprint("time to break")
							break
						# frappe.errprint("adding to max dis")
						max_allowed_dis = max_allowed_dis +x['rate']
						counter = counter + 1
				frappe.errprint("max_allowed_dis:"),
				frappe.errprint(max_allowed_dis)
				ext_item_amount = total_amount - max_allowed_dis
				frappe.errprint("ext_item_amount:"),
				frappe.errprint(ext_item_amount)
				dis = total_amount - p.for_amount -ext_item_amount
				frappe.errprint("discount"),
				frappe.errprint(dis)
				min_amount_list[p.base] = dis
